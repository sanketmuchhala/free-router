...
import { Health } from '../src/rank.js';
...
describe('OpenAI-compatible endpoint', () => {
  it('streams from the best model, falling back when one is rate limited, and names the model that answered', async () => {
    ...
    const { data: stream, response } = await client.chat.completions.create({ model: 'free-router/auto', stream: true, messages: [{ role: 'user', content: prompt }] }).withResponse();
    let text = '';
    for await (const part of stream) text += part.choices[0]?.delta?.content ?? '';
    expect(text).toBe('Hello from ok-8b.');
    expect(response.headers.get('x-free-router-model')).toBe('local/ok-8b');
    expect(response.headers.get('x-free-router-attempts')).toBe('2');
    ...
    await client.chat.completions.create({ model: 'free-router/auto', messages: [{ role: 'user', content: again }] });
  ...
  it('answers without streaming, and passes tool calls through unchanged', async () => {
    ...
    const plain = await client.chat.completions.create({ model: 'free-router/auto', messages: [{ role: 'user', content: 'non-stream please' }] });
    ...
    const stream = await client.chat.completions.create({ model: 'free-router/auto', stream: true, tools, messages: [{ role: 'user', content: 'read a.txt' }] });
    let args = '';
    let name = '';
    let finish = '';
  ...
  it('moves on from a model that sends nothing, or an error before any output, but never after output', async () => {
    ...
    const stream = await client.chat.completions.create({ model: 'free-router/auto', stream: true, messages: [{ role: 'user', content: prompt }] });
    let text = '';
    for await (const part of stream) text += part.choices[0]?.delta?.content ?? '';
    expect(text).toBe('Hello from ok-8b.');
    ...
    const response = await fetch(`${broken.base}/v1/chat/completions`, {
      ...
      body: JSON.stringify({ model: 'free-router/auto', stream: true, messages: [{ role: 'user', content: cut }] }),
    });
  ...
  it('says when no model can answer, with when to retry', async () => {
    ...
    const error = await client.chat.completions.create({ model: 'free-router/auto', messages: [{ role: 'user', content: 'nobody home' }] }).catch(e => e);
    ...
    const next = await client.chat.completions.create({ model: 'free-router/auto', messages: [{ role: 'user', content: 'still nobody' }] }).catch(e => e);
  ...
  it('keeps a conversation on the model it started with', async () => {
    ...
    await client.chat.completions.create({ model: 'free-router/auto', messages });
    expect(asked('sticky task')).toEqual(['limit-70b', 'ok-8b']);
    now += 10 * 60_000; // the 70B model's cooldown is over and it ranks first again
    await client.chat.completions.create({ model: 'free-router/auto', messages: [...messages, { role: 'assistant', content: 'ok' }, { role: 'user', content: 'sticky next turn' }] });
  ...
  it('requires a key, lists models, takes a model by name, and routes unknown names', async () => {
    ...
    expect(list.data.map(m => m.id)).toEqual(['free-router/auto', 'local/ok-3b', 'local/ok-8b']);
