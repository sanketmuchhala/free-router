...
import { homeDir, loadConfig } from './config.js';
import { KeyStore } from './keys.js';
import { ConfigError } from './providers.js';
import { Health, profileTask, rank, Sessions } from './rank.js';
import { createServer } from './server.js';

const HELP = `free-router: one endpoint for every free AI model you have.

Usage:
  free-router serve [--port N] [--host H] [--config FILE]   Start the router
  free-router models [--config FILE]                        List the free models it found, best first
  free-router key create [NAME]                             Make an API key for a client (shown once)
  free-router key list                                      List keys
  free-router key revoke ID                                 Revoke a key
  free-router init                                          Write an example config to ~/.free-router/config.json
...
async function serve(args: string[]) {
  ...
  if (!catalog.models().length) console.log('\nNo free models yet. Add a provider key (see free-router --help); the router keeps checking.');
  ...
  server.listen(port, host, () => {
    ...
    console.log(`\nfree-router listening on ${base}
  OpenAI clients:    base URL ${base}/v1, model free-router/auto
...
function key(args: string[]) {
  ...
  if (command === 'create') {
    ...
    if (!records.length) console.log('No keys. Create one with: free-router key create');
